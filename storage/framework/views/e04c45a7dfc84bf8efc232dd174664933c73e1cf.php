<!-- ========== Left Sidebar Start ========== -->
<div class="vertical-menu">
    <div data-simplebar class="h-100">
        <!--- Sidemenu -->
        <div id="sidebar-menu">
            <!-- Left Menu Start -->
            <ul class="metismenu list-unstyled" id="side-menu">
                <li class="menu-title" key="t-menu">Superação</li>
                <li class="mm-active">
                    <a href="javascript: void(0);" class="waves-effect">
                        <i class="bx bx-bookmark-minus"></i>
                        <span key="t-dashboards">Dashboard</span>
                    </a>
                    <ul class="sub-menu" aria-expanded="true">
                        <li><a href="./index" key="t-pagina-inicial"><i class="bx bx-home"></i>Página inicial</a></li>
                        <li><a href="./ranking-geral" key="t-ranking"><i class="bx bx-bar-chart-alt-2 bx-tada-hover"></i>Ranking Geral</a></li>
                        <li><a href="./comparador" key="t-comparador"><i class="bx bx-git-compare bx-tada-hover"></i>Comparador</a></li>
                        <li><a href="./simulador" key="t-simulador"><i class="bx bx-calculator bx-tada-hover"></i>Simulador</a></li>
                        <li><a href="./regulamento" key="t-regulamento"><i class="bx bx-book-open bx-tada-hover"></i>Regulamento</a></li>
                    </ul>
                </li>
            </ul>
        </div>
        <!-- Sidebar -->
    </div>
</div>
<!-- Left Sidebar End --><?php /**PATH C:\Users\mateus.g.0999\Documents\GitHub\template-plataforma-laravel\Admin\resources\views/layouts/sidebar.blade.php ENDPATH**/ ?>